...
async function renderTimeSlots() {
  ...
  // Show loading
  container.innerHTML = `<div class="time-placeholder">⏳ ${window.STRINGS?.loadingSlots || 'Завантаження...'}</div>`;

  let availableSlots = null;

  try {
    const res = await fetch(`/.netlify/functions/get-slots?date=${state.selectedDate}`);
    const data = await res.json();
    if (res.ok && Array.isArray(data.available)) {
      availableSlots = data.available;
    }
  } catch (err) {
    console.warn('Could not fetch slots from Cal.com, showing all:', err);
  }

  // Fallback: generate all slots if API failed
  if (!availableSlots) {
    availableSlots = [];
  ...
  if (availableSlots.length === 0) {
    container.innerHTML = `<div class="time-placeholder">${window.STRINGS?.noSlots || 'На цей день немає вільного часу'}</div>`;
    return;
  }
