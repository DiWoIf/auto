...
const CONFIG = {
  ...
  calUsername: "dmitry-diwolf-kazakov-er9woz",
...
function renderServices() {
  ...
  grid.innerHTML = window.SERVICES.map(
    (s, i) => `
...
function renderCalendar() {
  ...
  const dayNames = window.STRINGS?.days || [
    "Пн",
    "Вт",
    "Ср",
    "Чт",
...
async function handleSubmit() {
  ...
  try {
    const result = await submitViaNetlifyFunction();
    if (result?.success) {
      showSuccess();
    } else {
